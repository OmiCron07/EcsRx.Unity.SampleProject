...
namespace BindingsRx.Bindings
{
    public static class TextMeshExtensions
    {
        ...
        public static IDisposable BindFontSizeTo(this TMP_Text input, IReactiveProperty<float> property, params IFilter<float>[] filters)
        { return GenericBindings.Bind(() => input.fontSize, x => input.fontSize = x, property, BindingTypes.OneWay, filters).AddTo(input); }

        public static IDisposable BindFontSizeTo(this TMP_Text input, Func<float> getter, params IFilter<float>[] filters)
        { return GenericBindings.Bind(() => input.fontSize, x => input.fontSize = x, getter, null, BindingTypes.OneWay, filters).AddTo(input); }
        
        public static IDisposable BindColorTo(this TMP_Text input, IReactiveProperty<Color> property, BindingTypes bindingType = BindingTypes.Default, params IFilter<Color>[] filters)
        { return GenericBindings.Bind(() => input.color, x => input.color = x, property, bindingType, filters).AddTo(input); }

        public static IDisposable BindColorTo(this TMP_Text input, Func<Color> getter, Action<Color> setter, BindingTypes bindingType = BindingTypes.Default, params IFilter<Color>[] filters)
